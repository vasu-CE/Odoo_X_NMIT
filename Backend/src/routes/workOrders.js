...
router.get(
  ...
  [
    ...
    query("manufacturingOrderId").optional().isString(),
...
router.get(
  "/shop-floor",
  authenticate,
  authorize("SHOP_FLOOR_OPERATOR", "MANUFACTURING_MANAGER"),
  async (req, res) => {
    try {
...
router.patch(
  "/:id/start",
  authenticate,
  authorize("SHOP_FLOOR_OPERATOR", "ADMIN"),
  async (req, res) => {
    try {
      const { id } = req.params;
  ...
  async (req, res) => {
    try {
      ...
      if (currentWorkOrder.pausedAt) {
        additionalPausedTime = Math.floor(
          (now - currentWorkOrder.pausedAt) / (1000 * 60)
        ); // minutes
      }
      ...
      const workOrder = await prisma.workOrder.update({
        ...
        data: {
          status: "IN_PROGRESS", // Keep as IN_PROGRESS since we don't have PAUSED in our enum
          pausedAt: now,
...
router.post(
  "/:id/resume",
  authenticate,
  authorize("SHOP_FLOOR_OPERATOR", "MANUFACTURING_MANAGER", "ADMIN"),
  async (req, res) => {
    try {
      const { id } = req.params;
      ...
      const workOrder = await prisma.workOrder.update({
        where: {
          id,
          status: "IN_PROGRESS", // Since we don't have PAUSED status
        },
        data: {
          status: "IN_PROGRESS",
          pausedAt: null,
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const now = new Date();
      const startTime = currentWorkOrder.startedAt;
      ...
      // Use transaction to update work order and handle stock movements
      const result = await prisma.$transaction(async (tx) => {
        ...
        const workOrder = await tx.workOrder.update({
          ...
          data: {
            status: "DONE",
            completedAt: now,
            actualTimeMinutes: realDuration,
          },
          include: {
            manufacturingOrder: {
              include: {
        ...
        // Note: Material consumption is handled when manufacturing order is marked as DONE
