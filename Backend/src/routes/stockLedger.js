    ...
    try {
      ...
      const productsWithStock = await Promise.all(
        products.map(async (product) => {
          ...
          const freeToUse = Math.max(
            0,
            product.currentStock - product.reorderPoint
          );
          const totalValue =
            product.currentStock * (product.purchasePrice || 0);
  ...
  try {
    ...
    for (const workOrder of workOrders) {
      ...
      if (workOrder.status === "DONE") {
          ...
          if (consumedQuantity > 0) {
            const movement = await prisma.stockMovement.create({
              data: {
                ...
                totalValue:
                  consumedQuantity * (material.product.purchasePrice || 0),
        ...
        const isLastWorkOrder = allWorkOrders.every(
          (wo) => wo.id === workOrder.id || wo.status === "DONE"
        );
          ...
          const finishedGoodMovement = await prisma.stockMovement.create({
            data: {
              ...
              totalValue:
                ...
                (manufacturingOrder.product.purchasePrice || 0),
