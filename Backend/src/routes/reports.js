...
// @access  Private
router.get('/production-summary', authenticate, [
...
// @access  Private
router.get('/resource-utilization', authenticate, [
  query('startDate').optional().isISO8601(),
  query('endDate').optional().isISO8601(),
  ...
  try {
    ...
    const workCentersWithUtilization = workCenters.map(wc => {
      ...
      const completedWorkOrders = wc.workOrders.filter(wo => wo.status === 'COMPLETED').length;
      const utilization = wc.capacity > 0 ? (activeWorkOrders / wc.capacity) * 100 : 0;
      
      const totalEstimatedTime = wc.workOrders.reduce((sum, wo) => sum + (wo.estimatedTimeMinutes || 0), 0);
      const totalActualTime = wc.workOrders.reduce((sum, wo) => sum + (wo.actualTimeMinutes || 0), 0);
...
// @access  Private
router.get('/inventory-valuation', authenticate, [
  ...
  try {
    ...
    const productsWithValues = products.map(product => {
      const stockValue = product.salesPrice ? product.currentStock * product.salesPrice : 0;
      const costValue = product.purchasePrice ? product.currentStock * product.purchasePrice : 0;
    ...
    const totalStockValue = productsWithValues.reduce((sum, p) => sum + p.stockValue, 0);
    const totalCostValue = productsWithValues.reduce((sum, p) => sum + p.costValue, 0);
    const lowStockCount = productsWithValues.filter(p => p.isLowStock).length;
...
// @access  Private
router.get('/work-order-performance', authenticate, [
  query('startDate').optional().isISO8601(),
  query('endDate').optional().isISO8601(),
  query('workCenterId').optional().isString(),
  ...
  try {
    ...
    const performanceByWorkCenterWithDetails = await Promise.all(
      performanceByWorkCenter.map(async (item) => {
        ...
        const efficiency = item._sum.estimatedTimeMinutes > 0 
          ? (item._sum.actualTimeMinutes / item._sum.estimatedTimeMinutes) * 100 
          : 0;
    ...
    const avgEfficiency = workOrders
